...
- В корне лежит устаревшая vanilla-JS реализация (`app.js`), которая больше не запускается напрямую. Актуальный код — в `src/App.jsx`.
...
│   └── App.jsx           # Единственный корневой компонент (~1200 строк)
├── styles.css            # Глобальные стили (~1600 строк, CSS-переменные, темы)
├── app.js                # Устаревшая vanilla-JS реализация (не использовать)
...
- В `vite.config.js` настроен прокси: `/api` → `http://111.88.112.76:18080`.
...
1. **Настройка** (`select_base`, `select_user`, `select_source`)
...
- При старте пробуется Bitrix-режим (`{ BitrixID: "1657181189" }`); при таймауте — fallback на стандартный старт.
...
- **`app.js`** — полная vanilla-JS реализация визарда (чистый DOM, без React). Больше не является точкой входа. Оставлен для истории/референса. Не редактировать без явной договорённости.
...
- **API URL**: не хардкодить полный внешний URL (`http://111.88.112.76:18080/api`) в UI-поле при работе через dev-сервер — использовать `/api`.
- **BitrixID**: в коде захардкодан тестовый `BITRIX_USER_ID = "1657181189"` (Титов Михаил Сергеевич). Это тестовый режим; в реальном сценарии ID должен приходить из Bitrix24.
...
| `BITRIX_USER_ID` | `"1657181189"` | Тестовый BitrixID |
