...
- **Web-приложение** — авторизация по `Bitrix ID` (вводится вручную, см. раздел [«Авторизация в Web-версии»](#-авторизация-в-web-версии)).
...
2. **🔍 Поиск файлов конкретного пользователя:**
   - по введенному `BitrixID` backend находит только связанные с ним S3-ключи;
...
### **Что происходит после ввода BitrixID:**

1. Backend принимает `BitrixID`.
...
```mermaid
flowchart TD
    A[👤 Пользователь вводит BitrixID] --> B[🔄 Поиск связанных JSON в Yandex S3]
...
В отличие от Telegram-бота, web-приложение использует Bitrix ID пользователя, который вводится или передается в интеграции.

### Поток авторизации

1. На стартовом экране (или на шаге `select_user`) пользователь вводит **Bitrix ID**.
...
> `TelegramID` — совместимый алиас для старых клиентов. Основной формат для web/Bitrix — `BitrixID`.
...
|-----|-----------------------------|
| `access_denied` | Пользователь с указанным ID не найден. Проверьте корректность введенных данных. Если ID указан верно, обратитесь к ответственному сотруднику для помощи в регистрации. |
...
Backend считает доступ подтвержденным, если после S3-синхронизации найден хотя бы один профиль/база с переданным `BitrixID` или совместимым alias `TelegramID`.
