...
- **Авторизация web** — только Telegram/Bitrix ID, точечное обновление пользователя из Yandex S3 в AtWork при каждом входе
...
Текущая рабочая схема web:
- backend запущен на сервере `Backgosha` в Docker: `http://111.88.112.76:18080`;
- frontend запускается локально через Vite;
- `web_frontend/.env.local` задает `VITE_BACKEND_URL=http://111.88.112.76:18080`;
- все браузерные запросы `/api` проксируются локальным Vite dev server на серверный backend.

Если Telegram ID не найден, пользователь видит официальный сценарий регистрации: сообщение о том, что ID не найден, и кнопку «Помощь в регистрации» на профиль ответственного сотрудника.
