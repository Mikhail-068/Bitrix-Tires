...
- Прямые вызовы сервисов распознавания из Bitrix запрещены.
...
После действия `confirm_send` Bitrix не отправляет данные в 1С самостоятельно. Backend сам формирует итоговый JSON и отправляет его в сервис списания шин. Bitrix должен дождаться ответа backend и показать следующий `step`: `finished` при успешной отправке или текущий `confirm_send` с ошибкой `send_failed`, если 1С отклонила данные или сервис временно недоступен.
...
Bitrix не вызывает сервисы распознавания напрямую. Если после загрузки фото шаг не меняется, ориентируйтесь на `errors` в ответе backend и повторно рендерите текущий `step`.

Если проблема повторяется на корректных фото, передайте ответственным за backend:
- `session_id`;
- текущий `step`;
- код и текст ошибки из `errors[0]`.
...
Ориентируйтесь на `errors` в ответе backend. Чаще всего это означает, что фото не распознано, изображение невалидно или временно недоступен один из сервисов распознавания. В любом случае повторно рендерите экран по текущему `step` из ответа.
