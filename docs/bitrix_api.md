...
При старте сессии backend точечно обновляет данные введенного `BitrixID` из Yandex Bucket: скачивает только связанные с этим пользователем JSON-файлы в `AtWork/`, пересобирает индекс и уже после этого проверяет доступные базы. Это защищает от устаревших локальных данных при изменении базы или ссылки подключения.
